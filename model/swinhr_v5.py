...
# ==========================================================
# 【本次单一创新点】：级联去噪视觉提示器 (Sequential Denoising Prompt)
# ==========================================================
class SequentialDenoisingPrompt(nn.Module):
    """
    先通过通道注意力对血流特征进行“去噪/筛选”，
    再生成极度纯净的空间 Mask，用于单向提示解剖网络。
    """
    def __init__(self, channels, reduction=4):
        super().__init__()
        # 保证降维后的通道数至少为 4，防止浅层特征被过度压缩
        inter_channels = max(channels // reduction, 4)
        
        # 1. 通道去噪 (Channel Attention)
        ...
        # 2. 空间提示生成 (Spatial Attention)
        self.conv_spatial = nn.Sequential(
            nn.Conv2d(2, 1, kernel_size=7, padding=3, bias=False),
            nn.Sigmoid()
        )

    def forward(self, x_hemo, x_anatomy):
        # --- 步骤 A：血流特征自我净化 (筛选出肿瘤相关的通道) ---
        avg_out = self.fc(self.avg_pool(x_hemo))
        max_out = self.fc(self.max_pool(x_hemo))
        channel_weights = self.sigmoid_channel(avg_out + max_out)
        clean_hemo = x_hemo * channel_weights  # 抑制了背景噪声的高质量血流特征

        # --- 步骤 B：基于纯净血流生成空间提示 Mask ---
        # 沿着通道维度提取最大值和平均值，凸显显著的病灶区域
        avg_out_s = torch.mean(clean_hemo, dim=1, keepdim=True)
        max_out_s, _ = torch.max(clean_hemo, dim=1, keepdim=True)
        spatial_in = torch.cat([avg_out_s, max_out_s], dim=1)
        spatial_mask = self.conv_spatial(spatial_in)

        # --- 步骤 C：单向显式提示解剖特征 ---
        guided_anatomy = x_anatomy * spatial_mask + x_anatomy

        return guided_anatomy
...
class SwinHR(nn.Module):
    def __init__(
        ...
        # ==========================================================
        # 实例化级联去噪提示器 (代替原先瞎子一样的单通道卷积)
        # ==========================================================
        self.sdp4 = SequentialDenoisingPrompt(feature_size * 8)
        self.sdp3 = SequentialDenoisingPrompt(feature_size * 4)
        self.sdp2 = SequentialDenoisingPrompt(feature_size * 2)
        self.sdp1 = SequentialDenoisingPrompt(feature_size)
        self.sdp0 = SequentialDenoisingPrompt(feature_size)
    ...
    def forward(self, x):
        ...
        # 核心融合过程：级联去噪单向提示
        # ==========================================================
        guided_x3 = self.sdp4(x4_attention_up, x3.contiguous())
        guided_x2 = self.sdp3(x3_attention_up, x2.contiguous())
        guided_x1 = self.sdp2(x2_attention_up, x1.contiguous())
        guided_x0 = self.sdp1(x1_attention_up, x0.contiguous())
        guided_x5 = self.sdp0(x0_attention_up, x5.contiguous())

...
# ==========================================================
# 【本次单一创新点】：互相引导协同模块 (Mutual Co-Guidance Module)
# ==========================================================
class MutualGuidanceModule(nn.Module):
    """
    双向视觉提示模块：
    - 解剖特征(Anatomy) 提供锐利的边界信息，用来锐化血流特征。
    - 血流特征(Hemodynamics) 提供高响应的病灶定位，用来抑制解剖特征的背景。
    """
    def __init__(self, channels):
        super().__init__()
        # 解剖特征生成空间 Mask
        self.mask_gen_anatomy = nn.Conv2d(channels, 1, kernel_size=1)
        # 血流特征生成空间 Mask
        self.mask_gen_hemo = nn.Conv2d(channels, 1, kernel_size=1)

    def forward(self, feat_anatomy, feat_hemo):
        # 1. 互相生成 0~1 的提示权重图
        mask_a = torch.sigmoid(self.mask_gen_anatomy(feat_anatomy))
        mask_h = torch.sigmoid(self.mask_gen_hemo(feat_hemo))

        # 2. 交叉调制 (Residual 结构保证梯度稳定)
        # 血流引导解剖：定位核心
        guided_anatomy = feat_anatomy * mask_h + feat_anatomy
        # 解剖引导血流：利用结构锐化弥散边界
        guided_hemo = feat_hemo * mask_a + feat_hemo

        return guided_anatomy, guided_hemo
...
class SwinHR(nn.Module):
    def __init__(
        ...
        # ==========================================================
        # 实例化互相引导模块 (各尺度独立实例化，保证特征维度匹配)
        # ==========================================================
        self.mutual_guide4 = MutualGuidanceModule(feature_size * 8)
        self.mutual_guide3 = MutualGuidanceModule(feature_size * 4)
        self.mutual_guide2 = MutualGuidanceModule(feature_size * 2)
        self.mutual_guide1 = MutualGuidanceModule(feature_size)
        self.mutual_guide0 = MutualGuidanceModule(feature_size)
    ...
    def forward(self, x):
        ...
        # ==========================================================
        # 核心融合过程：双向互相提示 (Mutual Prompting)
        # ==========================================================
        guided_a4, guided_h4 = self.mutual_guide4(x3.contiguous(), x4_attention_up)
        guided_a3, guided_h3 = self.mutual_guide3(x2.contiguous(), x3_attention_up)
        guided_a2, guided_h2 = self.mutual_guide2(x1.contiguous(), x2_attention_up)
        guided_a1, guided_h1 = self.mutual_guide1(x0.contiguous(), x1_attention_up)
        guided_a0, guided_h0 = self.mutual_guide0(x5.contiguous(), x0_attention_up)

        # 互相提纯后，再进行拼接并送入主解码器
        x4_up = self.decoder5(x4, self.conv_block3(torch.cat((guided_a4, guided_h4), dim=1)))
        x3_up = self.decoder4(x4_up, self.conv_block2(torch.cat((guided_a3, guided_h3), dim=1)))
        x2_up = self.decoder3(x3_up, self.conv_block1(torch.cat((guided_a2, guided_h2), dim=1)))
        x1_up = self.decoder2(x2_up, self.conv_block0(torch.cat((guided_a1, guided_h1), dim=1)))
        x0_up = self.decoder1(x1_up, self.conv_block(torch.cat((guided_a0, guided_h0), dim=1)))        

...
# ==========================================================
# 核心网络架构：双流 SwinHR
# 包含创新点: Temporal Difference Encoding (时间差异编码) + 纯空间特征调制
# ==========================================================
class SwinHR(nn.Module):
    """
    Swin Transformer block is of the Swin UNETR model in MONAI
    Modified for Dual-Stream 2D Segmentation with Temporal Difference Encoding.
    """
    def __init__(
            self,
            img_size: Union[Sequence[int], int] = (256, 256),
            in_channels: int = 1,          # 原图流通道数 (VIBRANT)
            attn_channels: int = 8,        # 注意力流基础通道数 (SUB剪影序列)
            out_channels: int = 1,         
        ...
        # --- 分支 2：血流注意力流提取器 (加入时间差异) ---
        # 【创新点核心配置】：输入维度 = 原始 8 帧 + 相邻帧差值 7 帧 = 15 帧
        actual_attn_channels = attn_channels + (attn_channels - 1) if attn_channels > 1 else attn_channels
        
        self.swinViT_forward_attention = SwinTransformer(
            in_chans=actual_attn_channels, # <--- 接收拼接了时间差分的高维输入
            embed_dim=feature_size,
            window_size=window_size,
            patch_size=patch_size,
            depths=depths,
        ...
        # 保留你在 79.11% 版本中验证过的单通道纯空间调制层
        self.modulate_mask4 = nn.Conv2d(feature_size * 8, 1, kernel_size=1)
        self.modulate_mask3 = nn.Conv2d(feature_size * 4, 1, kernel_size=1)
        self.modulate_mask2 = nn.Conv2d(feature_size * 2, 1, kernel_size=1)
        self.modulate_mask1 = nn.Conv2d(feature_size, 1, kernel_size=1)
        self.modulate_mask0 = nn.Conv2d(feature_size, 1, kernel_size=1)
    ...
    def forward(self, x):
        ...
        """
        # --- 物理意义解耦 ---
        x_in = x[:, 0:1, :, :]         # [B, 1, H, W] 解剖流
        x_attention = x[:, 1:, :, :]   # [B, 8, H, W] 血流序列

        # ==========================================================
        # 【唯一新增的创新机制】：计算时间差异编码并拼接
        # ==========================================================
        if x_attention.shape[1] > 1:
            # diff_features 计算的是相邻两帧的时间梯度 (流入/流出速度)
            # diff = T_i - T_{i-1}
            diff_features = x_attention[:, 1:, :, :] - x_attention[:, :-1, :, :]
            # 将原始 8 帧与计算出的 7 帧差分拼接，作为 15 通道强时序特征送入 Transformer
            time_aware_attention = torch.cat([x_attention, diff_features], dim=1)
        else:
            time_aware_attention = x_attention
        # ==========================================================

        # 双流并行提取特征
        x0, x1, x2, x3, x4, x5 = self.swinViT_forward(x_in)
        # 注意这里喂入的是 time_aware_attention
        x0_attention, x1_attention, x2_attention, x3_attention, x4_attention, x5_attention = self.swinViT_forward_attention(time_aware_attention)
        ...
        # 1. 纯空间特征生成 (已验证有效)
        mask4 = torch.sigmoid(self.modulate_mask4(x4_attention_up))
        mask3 = torch.sigmoid(self.modulate_mask3(x3_attention_up))
        mask2 = torch.sigmoid(self.modulate_mask2(x2_attention_up))
        mask1 = torch.sigmoid(self.modulate_mask1(x1_attention_up))
        mask0 = torch.sigmoid(self.modulate_mask0(x0_attention_up))

        # 2. 空间调制
        guided_x3 = x3.contiguous() * mask4 + x3.contiguous()
        guided_x2 = x2.contiguous() * mask3 + x2.contiguous()
        guided_x1 = x1.contiguous() * mask2 + x1.contiguous()
        guided_x0 = x0.contiguous() * mask1 + x0.contiguous()
        guided_x5 = x5.contiguous() * mask0 + x5.contiguous()

        # 3. 调制后拼接融合
        x4_up = self.decoder5(x4, self.conv_block3(torch.cat((guided_x3, x4_attention_up), dim=1)))
        x3_up = self.decoder4(x4_up, self.conv_block2(torch.cat((guided_x2, x3_attention_up), dim=1)))
        x2_up = self.decoder3(x3_up, self.conv_block1(torch.cat((guided_x1, x2_attention_up), dim=1)))
        x1_up = self.decoder2(x2_up, self.conv_block0(torch.cat((guided_x0, x1_attention_up), dim=1)))
        x0_up = self.decoder1(x1_up, self.conv_block(torch.cat((guided_x5, x0_attention_up), dim=1)))        

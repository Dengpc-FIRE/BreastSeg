...
# ==========================================================
# 【颠覆性大改动 1】：边缘结构协同模块 (Spatio-Structural Synergy)
# ==========================================================
class SpatioStructuralSynergy(nn.Module):
    """
    统一解码器的核心引擎：
    - 解剖特征 (Anatomy) 提取高频结构，负责雕刻锐利边界。
    - 血流特征 (Hemo) 提取定位掩码，负责圈定肿瘤核心。
    """
    def __init__(self, channels):
        super().__init__()
        # 1. 深度可分离卷积提取解剖的高频结构 (极轻量)
    ...
    def forward(self, ana, hemo):
        structure = self.structure_conv(ana)
        location_mask = self.locator(hemo)
        
        # 核心融合法则：血流负责给核心高亮，解剖结构负责硬切边界！
        refined_hemo = (hemo * location_mask) + structure
        
        # 将无损的解剖特征与锐化后的血流拼接并降维
        return self.fuse(torch.cat([ana, refined_hemo], dim=1))


# ==========================================================
# 核心网络架构：单解码器协同网络 (USD-Net)
# ==========================================================
class SwinHR(nn.Module):
    def __init__(
        ...
        self.swinViT_forward = SwinTransformer(
            in_chans=in_channels, embed_dim=feature_size, window_size=window_size, patch_size=patch_size,
            depths=depths, num_heads=num_heads, mlp_ratio=4.0, qkv_bias=True, drop_rate=drop_rate,
            attn_drop_rate=attn_drop_rate, drop_path_rate=dropout_path_rate, norm_layer=nn.LayerNorm,
            use_checkpoint=use_checkpoint, spatial_dims=spatial_dims,
        )
        
        self.swinViT_forward_attention = SwinTransformer(
            in_chans=attn_channels, embed_dim=feature_size, window_size=window_size, patch_size=patch_size,
            depths=depths, num_heads=num_heads, mlp_ratio=4.0, qkv_bias=True, drop_rate=drop_rate,
            attn_drop_rate=attn_drop_rate, drop_path_rate=dropout_path_rate, norm_layer=nn.LayerNorm,
            use_checkpoint=use_checkpoint, spatial_dims=spatial_dims,
        )
        ...
        # ==========================================================
        # 【颠覆性大改动 2】：统一解码器 (Unified Decoder)
        # 彻底删除了原先冗余的 decoderX_attention 和海量拼接 conv_block
        # ==========================================================
        ...
        # 在多尺度跳跃连接处部署协同模块
        # ==========================================================
        self.syn4 = SpatioStructuralSynergy(feature_size * 16) # 瓶颈层
        self.syn3 = SpatioStructuralSynergy(feature_size * 8)
        self.syn2 = SpatioStructuralSynergy(feature_size * 4)
        self.syn1 = SpatioStructuralSynergy(feature_size * 2)
        self.syn0 = SpatioStructuralSynergy(feature_size)
        self.syn5 = SpatioStructuralSynergy(feature_size)
    ...
    def forward(self, x):
        ...
        h0, h1, h2, h3, h4, h5 = self.swinViT_forward_attention(x_hemo)
        
        # 2. 多尺度协同融合 (在每个特征级别，榨干两者的互补优势)
        fused_x4 = self.syn4(x4, h4) # 16C (Bottleneck)
        fused_x3 = self.syn3(x3, h3) # 8C
        fused_x2 = self.syn2(x2, h2) # 4C
        fused_x1 = self.syn1(x1, h1) # 2C
        fused_x0 = self.syn0(x0, h0) # C
        fused_x5 = self.syn5(x5, h5) # C

        # 3. 单主干极致纯净解码
        # 每一层严格接收上一层的上采样(inp) 和 当前层的优质协同特征(skip)
        up3 = self.decoder5(inp=fused_x4, skip=fused_x3)
        up2 = self.decoder4(inp=up3, skip=fused_x2)
        up1 = self.decoder3(inp=up2, skip=fused_x1)
        up0 = self.decoder2(inp=up1, skip=fused_x0)
        up5 = self.decoder1(inp=up0, skip=fused_x5)

...
# ==========================================================
# 核心网络架构：双流 SwinHR (修改适配 9通道 2D 任务)
# ==========================================================
class SwinHR(nn.Module):
    """
    Swin Transformer block is of the Swin UNETR model in MONAI
    Modified for Dual-Stream (Anatomy + Hemodynamics) 2D Segmentation.
    """
    def __init__(
            self,
            img_size: Union[Sequence[int], int] = (256, 256),
            in_channels: int = 1,          # 【修改 1】: 原图流通道数 (VIBRANT)
            attn_channels: int = 8,        # 【修改 2】: 新增注意力流通道数 (SUB剪影序列)
            out_channels: int = 1,         # 最终分割 Mask 通道数
            ...
            spatial_dims: int = 2,         # 【修改 3】: 默认改为 2 (2D图像)
    ) -> None:
        ...
        # --- 分支 2：血流注意力流提取器 (处理 attn_channels) ---
        self.swinViT_forward_attention = SwinTransformer(
            in_chans=attn_channels,      # 【关键修改】使用剪影流通道数
            embed_dim=feature_size,
            window_size=window_size,
            patch_size=patch_size,
            depths=depths,
    ...
    def forward(self, x):
        """
        【修改 4】: 修改 forward 函数，接收统一的 9 通道输入，并在内部拆分
        x: [B, 9, H, W]
        """
        # --- 物理意义解耦拆分 ---
        x_in = x[:, 0:1, :, :]         # [B, 1, H, W] 解剖流 (VIBRANT原图)
        x_attention = x[:, 1:, :, :]   # [B, 8, H, W] 血流流 (SUB剪影序列)
        ...
        # 1. 生成 0~1 的空间注意力权重图 (Spatial Prompts)
        ...
        # 2. 调制解剖特征 (解剖特征 * 血流Mask) + 原特征 (残差连接)
        # 这里 x3, x2, x1, x0, x5 分别是解剖流的 skip connection 特征
...
class SwinTransformer(nn.Module):
    def __init__(self, in_chans: int, embed_dim: int, window_size: Sequence[int], patch_size: Sequence[int], depths: Sequence[int], num_heads: Sequence[int], mlp_ratio: float = 4.0, qkv_bias: bool = True, drop_rate: float = 0.0, attn_drop_rate: float = 0.0, drop_path_rate: float = 0.0, norm_layer: Type[LayerNorm] = nn.LayerNorm, patch_norm: bool = False, use_checkpoint: bool = False, spatial_dims: int = 3) -> None:
        ...
        # 【修改 5】: 统一使用传入的 spatial_dims，解绑写死的 3D 卷积
...
class Up_Concat(nn.Module):
    def __init__(self, in_channels, spatial_dims=2): # 【优化】修正默认 3D 卷积的隐藏设定
        super().__init__()

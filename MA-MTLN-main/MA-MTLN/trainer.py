...
class Trainer(nn.Module):
    def __init__(self, fold, data_root, out_path, output_fold, input_channels=17):
    ...
    def initialize(self, training=True):
        ...
        if training:
            ...
            self.train_data, self.test_data = get_default_augmentation(self.train_data, self.test_data,
                                                                 ...
                                                                 self.data_aug_params)
        self.network = MTLN3D(in_channels=self.input_channels).cuda()
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        ...
        if data.shape[1] != self.input_channels:
            raise ValueError(
                "Expected %d input channels, got %d. The npz data should store "
                "image channels first and the segmentation label as the last channel."

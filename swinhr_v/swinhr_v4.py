...
# ==========================================================
# 【本次单一创新点】：跨流联合注意力门 (Cross-Stream Attention Gate)
# ==========================================================
class CrossStreamAttentionGate(nn.Module):
    """
    联合解剖特征(精确边界)和血流特征(高对比度定位)共同生成高质量的空间注意力 Mask。
    """
    def __init__(self, channels):
        super().__init__()
        # 将通道数压缩一半以减少参数量，至少保留 1 个通道
        inter_channels = max(channels // 2, 1)
        
        # W_a: 映射解剖特征
        self.W_a = nn.Conv2d(channels, inter_channels, kernel_size=1, stride=1, padding=0, bias=False)
        # W_h: 映射血流特征
        self.W_h = nn.Conv2d(channels, inter_channels, kernel_size=1, stride=1, padding=0, bias=False)
        
        self.relu = nn.ReLU(inplace=True)
        
        # Psi: 将联合特征映射为单通道的 Spatial Mask
        self.psi = nn.Conv2d(inter_channels, 1, kernel_size=1, stride=1, padding=0, bias=False)
    ...
    def forward(self, feat_anatomy, feat_hemo):
        # 1. 解剖和血流特征映射到同一子空间
        a_proj = self.W_a(feat_anatomy)
        h_proj = self.W_h(feat_hemo)
        
        # 2. 联合激活 (加法融合)
        # 【核心】：解剖的高频边界与血流的低频定位在这里发生物理碰撞
        joint_feat = self.relu(a_proj + h_proj)
        
        # 3. 生成极其精确的 Spatial Mask
        mask = self.sigmoid(self.psi(joint_feat))
        
        # 4. 调制解剖特征 (保留残差结构以防梯度消失)
        guided_anatomy = feat_anatomy * mask + feat_anatomy
        
        # 5. 拼接并做深度融合
        fused = self.fuse_conv(torch.cat([guided_anatomy, feat_hemo], dim=1))
        return fused
...
class SwinHR(nn.Module):
    def __init__(
        ...
        # 实例化联合注意力门 (多尺度分布)
        # ==========================================================
        self.csag4 = CrossStreamAttentionGate(feature_size * 8)
        self.csag3 = CrossStreamAttentionGate(feature_size * 4)
        self.csag2 = CrossStreamAttentionGate(feature_size * 2)
        self.csag1 = CrossStreamAttentionGate(feature_size)
        self.csag0 = CrossStreamAttentionGate(feature_size)
    ...
    def forward(self, x):
        ...
        # ==========================================================
        # 核心融合过程：联合注意力门
        # 解剖和血流特征共同参与 Mask 生成，获得清晰边界和准确定位
        # ==========================================================
        fused_x4 = self.csag4(x3.contiguous(), x4_attention_up)
        fused_x3 = self.csag3(x2.contiguous(), x3_attention_up)
        fused_x2 = self.csag2(x1.contiguous(), x2_attention_up)
        fused_x1 = self.csag1(x0.contiguous(), x1_attention_up)
        fused_x0 = self.csag0(x5.contiguous(), x0_attention_up)

        # 融合后的极品特征送入主解码器
        x4_up = self.decoder5(x4, fused_x4)
        x3_up = self.decoder4(x4_up, fused_x3)
        x2_up = self.decoder3(x3_up, fused_x2)
        x1_up = self.decoder2(x2_up, fused_x1)
        x0_up = self.decoder1(x1_up, fused_x0)        

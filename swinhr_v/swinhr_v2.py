...
# ==========================================================
# 【本次单一创新点】：多尺度上下文提示生成器 (Multi-Scale Prompt Generator)
# ==========================================================
class MultiScalePromptGen(nn.Module):
    """
    替代原本简单的 1x1 卷积。
    利用轻量级的空洞卷积 (Dilated Convolutions) 捕捉不同尺度的局部上下文，
    使得生成的空间 Mask 能够更好地覆盖大小不一的病灶，且边缘更精准。
    """
    def __init__(self, in_channels):
        super().__init__()
        # 1. 通道压缩 (Squeeze)：将几百个通道压缩到 16，极大地降低计算量，保证训练速度
        self.squeeze = nn.Conv2d(in_channels, 16, kernel_size=1, bias=False)
        self.norm = nn.InstanceNorm2d(16)
        self.relu = nn.ReLU(inplace=True)
        
        # 2. 多尺度感受野提取 (使用 Depthwise 卷积，参数量极小)
        # 感受野 3x3
        self.branch_1 = nn.Conv2d(16, 16, kernel_size=3, padding=1, groups=16, bias=False)
        # 感受野 5x5 (通过 dilation=2 放大视野)
        self.branch_2 = nn.Conv2d(16, 16, kernel_size=3, padding=2, dilation=2, groups=16, bias=False)
        # 感受野 7x7 (通过 dilation=3 进一步放大视野)
        self.branch_3 = nn.Conv2d(16, 16, kernel_size=3, padding=3, dilation=3, groups=16, bias=False)
        
        # 3. 融合输出单通道空间概率图
        self.out_conv = nn.Conv2d(16 * 4, 1, kernel_size=1)

    def forward(self, x):
        # 压缩通道并激活
        x_sq = self.relu(self.norm(self.squeeze(x)))
        
        # 多尺度分支提取
        b1 = self.branch_1(x_sq)
        b2 = self.branch_2(x_sq)
        b3 = self.branch_3(x_sq)
        
        # 拼接多尺度特征 (16 + 16 + 16 + 16 = 64 channels)
        out = torch.cat([x_sq, b1, b2, b3], dim=1)
...
class SwinHR(nn.Module):
    def __init__(
        ...
        # ==========================================================
        # 实例化【多尺度上下文提示生成器】替换原本瞎子一样的 1x1 conv
        # ==========================================================
        self.modulate_mask4 = MultiScalePromptGen(feature_size * 8)
        self.modulate_mask3 = MultiScalePromptGen(feature_size * 4)
        self.modulate_mask2 = MultiScalePromptGen(feature_size * 2)
        self.modulate_mask1 = MultiScalePromptGen(feature_size)
        self.modulate_mask0 = MultiScalePromptGen(feature_size)
    ...
    def forward(self, x):
        ...
        # 1. 经过多尺度感受野强化的高级空间 Prompt
        mask4 = self.modulate_mask4(x4_attention_up)
        mask3 = self.modulate_mask3(x3_attention_up)
        mask2 = self.modulate_mask2(x2_attention_up)
        mask1 = self.modulate_mask1(x1_attention_up)
        mask0 = self.modulate_mask0(x0_attention_up)

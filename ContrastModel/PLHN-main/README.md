...
## BreastSeg fixed-split adaptation

This copy contains a local adapter for the current BreastSeg project under
`reproduce_plhn/`.
...
The adapter reads `../seg/{train,val,test}`, converts it into PLHN-style 3D
NIfTI cases under `reproduce_plhn/BreastDM_PLHN_3D`, and writes experiment
outputs under `reproduce_plhn/results_plhn_breastdm_3d`.

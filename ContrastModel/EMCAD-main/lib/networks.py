...
class EMCADNet(nn.Module):
    def __init__(self, num_classes=1, kernel_sizes=[1,3,5], expansion_factor=2, dw_parallel=True, add=True, lgag_ks=3, activation='relu', encoder='pvt_v2_b2', pretrain=True, pretrained_dir='./pretrained_pth/pvt/', in_channels=3):
    ...
    def _adapt_input_channels(self, in_channels):
        if in_channels == 3:
            return
        if hasattr(self.backbone, 'patch_embed1') and hasattr(self.backbone.patch_embed1, 'proj'):
            ...
            nn.init.kaiming_normal_(self.backbone.patch_embed1.proj.weight, nonlinearity='relu')
            if self.backbone.patch_embed1.proj.bias is not None:
                nn.init.zeros_(self.backbone.patch_embed1.proj.bias)
            return
        if hasattr(self.backbone, 'conv1'):
            old = self.backbone.conv1
            if isinstance(old, nn.Sequential):
                first = old[0]
                old[0] = nn.Conv2d(
                    in_channels,
                    first.out_channels,
                ...
                nn.init.kaiming_normal_(old[0].weight, nonlinearity='relu')
            else:
            ...
            return
        raise RuntimeError('Unsupported EMCAD backbone input stem for 17-channel adaptation')

...
def process_patient(img_patient_dir, lbl_patient_dir, out_data_dir, out_gt_dir, size):
    ...
    # 核心修改：只锁定 SUB2 文件夹作为标签来源
    dir_label_target = lbl_patient_dir / "SUB2"
    ...
    if not dir_label_target.exists():
        # 如果这个病人没有 SUB2 的标签文件夹，说明无法训练，跳过
        # print(f"Skip {patient_id}: No SUB2 label folder")
        return
    ...
    for slice_path in slices:
        ...
        # --- B. 获取标签 (从 labels/../SUB2 中找) ---
        # 假设标签文件名和原图文件名一致 (p-032.png 对应 p-032.jpg)
        # 或者标签可能是 png，原图是 jpg，尝试寻找同名文件
...
def main():
    ...
    parser.add_argument('--output_root', type=str, default='./processed_9ch_sub2label', help='输出目录')
    ...
    print(f"标签数据 (SUB2 .png) 保存在: {output_root}/<split>/GT")
